...
<tr>
<th width="8%">순위</th><th width="20%">이미지</th><th width="35%">아이디/내용</th><th width="*">우승비율<br>(우승 횟수/전체 게임 수)
</th>
</tr>
...
<tr>
<td>${i.count}</td>
<td><img width="100%;" height="250px;" style="cursor: pointer" onclick="location.href='adStyleView?siidx=${wd.getSi_idx() }&piid=${wd.getPi_id()}'" src="resources/img/style_img/${wd.getSi_img()}"/></td>
<td>${wd.getMi_id() }<br>${wd.getSi_content()}</td>
<td><c:if test="${late>0}">
<meter style="width:80%;height:40px; " min="0" max="100" value="<fmt:formatNumber value="${late*100}" pattern=".0"/>" ></meter><br>
<fmt:formatNumber value="${late*100}" pattern=".0"/></c:if> 
<c:if test="${late<=0}">
<meter style="width:80%;height:40px; " min="0" max="100" value="0" ></meter><br>
0.0</c:if>%
