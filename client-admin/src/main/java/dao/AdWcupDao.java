...
public class AdWcupDao {
	...
	public List<EventWcupInfo> getEvtWcupList() {
		...
		public List<WcupDetail> getEventWcupDList(int ewidx){
			String sql ="select  a.mi_id,ewl_win, b.si_img, b.pi_id,b.si_idx, b.si_content from  t_evt_wcup_list a, t_style_info b where a.si_idx = b.si_idx  and ew_idx='"+ewidx+"' order by ewl_win desc";
			...
			List <WcupDetail> detailList = jdbc.query(sql, new RowMapper<WcupDetail>(){
				public WcupDetail mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					wd.setEwl_win(rs.getInt("ewl_win"));					wd.setMi_id(rs.getString("mi_id"));
					wd.setSi_img(rs.getString("si_img"));					wd.setSi_idx(rs.getString("si_idx"));
					wd.setPi_id(rs.getString("Pi_id"));						wd.setSi_content(rs.getString("si_content"));
					return wd;
				}			
			});
